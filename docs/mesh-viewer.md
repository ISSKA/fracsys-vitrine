...
├── fracture_zone_loader.ts      # Fracture-zone loading and layers
├── damage_zone_loader.ts        # Zone switching
├── scalar_bar.ts                # DOM-based colour bar overlay
├── vtk.d.ts                     # Minimal VTK.js declarations
└── styles.css
```
...
The nginx container serves the unified root `dist/` directory on port 80.
...
| `npm run type-check` | Check all application types without emitting |
